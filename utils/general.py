...
def labels_to_class_weights(labels, nc=80):
    ...
    classes = labels[:, 0].astype(np.int32)  # labels = [class xywh]  classes : size=(866643)
...
def scale_labels(img1_shape, labels, img0_shape, ratio_pad=None):
    ...
    for i, label in enumerate(labels):
        ...
        # add the following for detect
        irect = (rect[0][0].int().item(),rect[0][1].int().item()),(rect[1][0].int().item(),rect[1][1].int().item()),rect[2].int().item()
        # print(irect)
        poly = cv2.boxPoints(irect)  # 返回rect对应的四个点的值 normalized
...
def longsideformat2poly(x_c, y_c, longside, shortside, theta_longside):
    ...
    # print(f"type of rect: {rect}")
    # poly = [(x1,y1),(x2,y2),(x3,y3),(x4,y4)]
    
    #print('rect_0',rect[0])
    #print('rect_1',rect[1])
    #print('rect_2',rect[2].int())

    irect=(rect[0][0].int().item(),rect[0][1].int().item()),(rect[1][0].int().item(),rect[1][1].int().item()),rect[2].int().item()
    # print(type(irect[0][0]))
    # print("-----------")
    #detect:
    # poly = np.double(cv2.boxPoints(((10,10),(irect[1][0],irect[1][1]),irect[2])))  # 返回rect对应的四个点的值 normalized
    # bp = cv2.boxPoints(((314., 157.0), (66.3, 31), -89))
    poly = np.double( cv2.boxPoints(irect) )
...
def LongSideToPoly(xc, yc, w, h, theta1):
    # xc = xc_n * W
    # yc = yc_n * H
...
def VisualizeLabelsNP(img, labels, name, output_path):
    H, W, C = img.shape
    for label in labels:
    ...
    cv2.putText(img, name, (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,0,0), 2, cv2.LINE_AA)
...
def plot_one_rotated_box(rbox, img, color=None, label=None, line_thickness=None, pi_format=True):
    ...
    # rect=[(x_c,y_c),(w,h),Θ] Θ:flaot[0-179]  -> (-180,0)
    # print(rbox[0], rbox[1], rbox[2], rbox[3], rbox[4])
    # x1, y1, x2, y2, x3, y3, x4, y4 = LongSideToPoly(rbox[0].cpu(), rbox[1].cpu(), rbox[2].cpu(), rbox[3].cpu(), rbox[4].cpu())
    x1, y1, x2, y2, x3, y3, x4, y4 = LongSideToPoly(rbox[0], rbox[1], rbox[2], rbox[3], rbox[4])

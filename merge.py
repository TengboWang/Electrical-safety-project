...
def mergebase(srcpath, dstpath, nms):
    ...
    for fullname in filelist:  # 'example_split/../P0001.txt'
        ...
        with open(fullname, 'r') as f_in:
            ...
            for splitline in splitlines:  # splitline:每行中的m个元素
                ...
                ori_pic_name.append(oriname)#ori_pic_name
    ...
    # pre_merge_path='./view_total_picture/labelTxt/pre_merge_classname.txt' 
    # k=open(pre_merge_path,'a+')
    # k.read()
    #os.rename(pre_merge_path,'./view_total_picture/labelTxt/'+ori_pic_name[0]+'.txt')
...
def draw_DOTA_image(imgsrcpath, imglabelspath, dstpath, extractclassname, thickness):
    ...
    label_files_name=[]
    pic_files_name=[]
    for root, dirs, files in os.walk(imglabelspath, topdown=False):#获得路径下的文件名
        label_files_name=files
    for root, dirs, files in os.walk(imgsrcpath, topdown=False):#获得路径下的文件名
        pic_files_name=files          
   
    #统计芯片的个数
    #txt_filename = open(imglabelspath+'/'+label_files_name[0]) 
    #total_chip_num = len(txt_filename.readlines()) 
    #chip_num_text='the total chips ='+' '+str(total_chip_num)
    
    #print(total_chip_num)
    #print('imglabelspath=',imglabelspath)
    if not os.path.exists(dstpath):
        os.makedirs(dstpath)
    ...
    for fullname in filelist:  # fullname='/.../P000?.txt'
        ...
        for i, obj in enumerate(objects):
            ...
            cv2.putText(img, classname, (poly[0][0], poly[0][1]), cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2)
            
      	
        cv2.putText(img, chip_num_text, (600, 600), cv2.FONT_HERSHEY_PLAIN, 10, (0, 0, 255), 20)
...
if __name__ == '__main__':
    ...
    mergebypoly(r'/home/huasi/AllNeedCopy_datasets/dianli/YOLOv5_DOTA_OBB-master/inference/detect_images/result_txt/result_before_merge',
                r'/home/huasi/AllNeedCopy_datasets/dianli/YOLOv5_DOTA_OBB-master/inference/detect_images/result_txt/result_after_merge')
    print('检测结果已merge')
    
    classnames_chip=['front','back']

    draw_DOTA_image(imgsrcpath=r'/home/huasi/AllNeedCopy_datasets/dianli/YOLOv5_DOTA_OBB-master/inference/pre_images',
                    imglabelspath=r'/home/huasi/AllNeedCopy_datasets/dianli/YOLOv5_DOTA_OBB-master/inference/detect_images/result_txt/result_after_merge',
                    dstpath=r'/home/huasi/AllNeedCopy_datasets/dianli/YOLOv5_DOTA_OBB-master/inference/merge_images',
                    extractclassname=classnames_chip,
                    thickness=5
                    )
